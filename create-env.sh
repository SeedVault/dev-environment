...
if [ ! -e "./.env" ]; then
    ...
    export SECURE_BLOCKSCOUT_SECRETS_SYSTEM=$(export LC_CTYPE=C; cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
    sed -e "s/secure_redis_password/${SECURE_REDIS_PASSWORD}/g" \
        ...
        -e "s/secure_blockscout_secrets_system/${SECURE_BLOCKSCOUT_SECRETS_SYSTEM}/g" \
        ./.env.example > ./.env
